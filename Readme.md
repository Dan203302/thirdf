...
flowchart LR
  subgraph Client["Браузер пользователя"]
    UI["Dashboard<br/>(Next.js / React)"]
  end

  subgraph Web["Next.js фронтенд"]
    NEXT["Маршруты Next.js:<br/>/ /iss /legacy /api/*"]
  end
...
- **next_web** — фронтенд на Next.js (React + TypeScript + Tailwind): dashboard, ISS-карта и графики (Leaflet + Chart.js через CDN), JWST-галерея, страница legacy и API-прокси к rust_iss и внешним сервисам.
- **iss_db** — PostgreSQL (хранение логов, кэшей и производных данных).
- **pascal_legacy** — легаси-утилита (Pascal, FreePascal), периодически генерирует CSV (`recorded_at, is_ok, voltage, temp, source_file`) и импортирует данные в таблицу `telemetry_legacy`.
- **nginx** — фронтовой reverse-proxy для прод-окружения; в текущем docker-compose для разработки запросы идут напрямую в `next_web`.
...
## Архитектура после рефакторинга (фактическое решение)

- **rust_iss** — сервис на Rust (Axum + SQLx), опрашивает внешние API (ISS, NASA OSDR, JWST, AstronomyAPI), пишет логи и производные данные в PostgreSQL и предоставляет REST-ручки `/health`, `/last`, `/fetch`, `/osdr/*`.
- **next_web** — фронтенд на Next.js (React + TypeScript + Tailwind):
  - главная страница `/` с картой орбиты ISS (Leaflet), графиками скорости/высоты (Chart.js) и JWST-блоком (выбор избранного наблюдения из галереи);
  - страница `/iss` с табличным трендом по орбите;
  - страница `/legacy` для просмотра данных легаси-модуля с фильтрами и экспортом в XLSX;
  - API-прокси-ручки `/api/iss/*`, `/api/jwst/feed`, `/api/astro/events`, `/api/legacy/*`.
- **pascal_legacy** — контейнер с утилитой на FreePascal, периодически генерирует CSV c полями `recorded_at, is_ok, voltage, temp, source_file` и импортирует их в таблицу `telemetry_legacy` в PostgreSQL.
- **iss_db** — PostgreSQL: хранит таблицы `iss_fetch_log`, `osdr_items`, `telemetry_legacy` и вспомогательные кэши.
...
2. После старта:
   ...
   - Основные страницы:
     - `/` — дашборд (ISS карта и графики, JWST-галерея и избранное наблюдение).
     - `/iss` — тренд по орбите ISS.
     - `/legacy` — данные из Pascal-легаси с фильтрами и выгрузкой в XLSX.